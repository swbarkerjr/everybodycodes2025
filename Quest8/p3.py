def main():
    data = readfile('/home/steve/everybodycodes/2025/Quest8/part3.txt')
    nail_count = 256
    count = 0
    max_count = 0
    tested_pairs = list()
    ...
    #create cut imaginary line
    for x1 in range(1, int(nail_count+1)):  #only need 1/2 circle
        for x2 in range(x1+1, int(nail_count+1)):
            x = [x1,x2]                 
            pass
            for y in pairs:
                if x[0] in y and x[1] in y:  #cut shares same line as existing
                    count += 1
                elif x[0] in y:  #if not same line, does cut shares vertex - start
                    continue
                elif x[1] in y:  #if not same line, cut shares vertex - end
                    continue
                elif middle_crossing(x,y,nail_count):   #if not same line, does cut midpoint cross check
                    count += 1
                else:
            ...
            if count > max_count:
                max_count = count
                print(f'cut: {x}, count: {count}')
            count = 0         
                        
    
    
    print(f'part3: {max_count}')
